...
(function () {
  ...
  class FlutterServiceWorkerLoader {
    ...
    loadServiceWorker(settings) {
      ...
      // Bypass the secure context requirement in non-secure contexts (e.g., HTTP)
      if (!window.isSecureContext) {
        console.warn("Service Worker not loaded as the current context is not secure (HTTP).");
        return Promise.resolve();
      }
      // Continue with service worker loading for secure contexts (HTTPS)
      const {
  ...
  class FlutterLoader {
    ...
    async loadEntrypoint(options) {
        ...
        // Load the service worker only in a secure context
        if (window.isSecureContext) {
